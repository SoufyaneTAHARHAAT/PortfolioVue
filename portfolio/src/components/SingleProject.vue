...
const handleScroll = () => {
    for(let i = 0; i< fadeInElements.value.length; i++) {
        ...
        if(isElemVisible(elem)) {
            ...
            fadeInElements.value.splice(i, 1) //to be animated once
        }
    }
}
...
const isElemVisible = (el) => {
    ...
    return elemTop < window.innerHeight && elemBottom;
}
...
<template>
   <div class="project">
    <div class="left fade-in" :class="reverse ? 'reverse' : ''">
        ...
        <a :href="`/src/assets/projects/${project.image}.png`" target="_blanck">
            <img :src="`/src/assets/projects/${project.image}.png`" :alt="project.name">
        </a>
    </div>
